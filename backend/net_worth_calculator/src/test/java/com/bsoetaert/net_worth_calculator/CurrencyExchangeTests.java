...
public class CurrencyExchangeTests {
    ...
    public void updateRatesFailsAndUsesOldValues() {
        ...
        rate1.addStartCurrency(baseCurrency, exchangeRate1);
        Date today = new Date();
        LocalDateTime ldt = LocalDateTime.ofInstant(today.toInstant(), ZoneId.systemDefault()).minusDays(1);
        Date oneDayAgo = Date.from(ldt.atZone(ZoneId.systemDefault()).toInstant());
